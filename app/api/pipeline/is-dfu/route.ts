...
export async function POST(request: Request) {
  try {
    ...
    const modelPath = join(process.cwd(), 'scripts', 'is-dfu.pth');
    const scriptPath = join(process.cwd(), 'scripts', 'is-dfu_inference.py');
    
    // Setup cache directory
    const cacheDir = join(process.cwd(), 'app', 'is_dfu_cache');
    ...
    try {
      ...
      const cachedPrediction = JSON.parse(cachedContent);
    ...
    const outputFilePath = join(tmpdir(), `is_dfu_output_${Date.now()}.json`);
    ...
    const prediction = JSON.parse(outputContent);
    
    // Save to cache
    try {
      await writeFile(cacheFilePath, outputContent, 'utf-8');
      console.log(`Cached prediction for ${imageBasename}`);
    } catch {
      console.error('Failed to cache prediction:', new Error('Could not write cache'));
    ...
    console.error('Error running is-dfu inference:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to run is-dfu inference' },
      { status: 500 }
    );
  }
}
